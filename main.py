...
# --- 投稿テキストの設定 ---
post_texts = {
    : ["もう1時！？時間早すぎ…", "深夜のネットサーフィンが止まらない", "誰か電話しない？", "コンビニ行きたい",
...
# 時間帯に応じた投稿を取得（アカウントごとに違う投稿を生成）
def get_time_based_post(seed_offset):
    now = datetime.now(jst).hour
    if now == 0:
        now = 24  # 0時を24時として扱う
    random.seed(time.time() + seed_offset)  # 各アカウントで違う乱数を生成
...
def time_based_posting_loop():
    while True:
        ...
        for index, (client, email) in enumerate(yay_clients):
            try:
                post = get_time_based_post(index)  # アカウントごとに異なる投稿を生成
