...
# ログイン処理
yay_clients = []
print(f"[{datetime.now(jst)}] 🌐 ログイン処理開始（{len(accounts)}アカウント）")
for account in accounts:
    try:
        client = yaylib.Client()
        client.login(account["email"], account["password"])
        yay_clients.append((client, account["email"]))  # client, email をペアで保存
        print(f"[{datetime.now(jst)}] ✅ {account['email']} でログイン成功")
    except Exception as e:
        print(f"[{datetime.now(jst)}] ❌ {account['email']} のログインに失敗: {e}")

if not yay_clients:
    print(f"[{datetime.now(jst)}] 🚨 ログインできたアカウントがありません。終了します。")
    exit(1)
...
post_texts = {
    ...
    13: ["午後の仕事スタート！", "ランチ後って眠くなるよね", "集中力が試される時間", "眠気と戦い中",
...
def time_based_posting_loop():
    while True:
        print(f"[{datetime.now(jst)}] 📢 自動投稿を開始")
        for client, email in yay_clients:
            try:
                post = get_time_based_post()
                client.create_post(post)
                print(f"[{datetime.now(jst)}] ✅ {email} が投稿: {post}")
            except Exception as e:
                print(f"[{datetime.now(jst)}] ❌ 投稿エラー ({email}): {e}")
        print(f"[{datetime.now(jst)}] ⏳ 次の投稿まで1時間待機")
